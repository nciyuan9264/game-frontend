...
const MessageSender: React.FC<MessageSenderProps> = ({ onMessageSend }) => {
  ...
  const content = (
    <div className={styles.popoverContent}>
      {PRESET_MESSAGES.map((item) => (
        <div key={item.audioType} className={styles.messageItem} onClick={() => handleSelect(item.audioType)}>
          {item.message}
        </div>
      ))}
    </div>
  ...
  return (
    <Popover content={content} trigger="click" placement="topRight" open={visible} onOpenChange={setVisible}>
      <Button
