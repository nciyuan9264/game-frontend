...
export const createTileGeometry = (id: string, tileSize: number, scene: Scene): Mesh => {
  const tile = MeshBuilder.CreateBox(`tile_${id}`, {
    size: tileSize,
    height: 0.3,
  }, scene);
...
export const createBoardBorders = (scene: Scene, tileSize: number, boardOffsetX: number, boardOffsetZ: number) => {
  ...
  // 创建四个边框
  const borders = [
    MeshBuilder.CreateBox('borderTop', {
      width: boardWidth + borderWidth * 2,
      height: 0.4,
      depth: borderWidth,
    }, scene),
    MeshBuilder.CreateBox('borderBottom', {
      width: boardWidth + borderWidth * 2,
      height: 0.4,
      depth: borderWidth,
    }, scene),
    MeshBuilder.CreateBox('borderLeft', {
      width: borderWidth,
      height: 0.4,
      depth: boardHeight,
    }, scene),
    MeshBuilder.CreateBox('borderRight', {
      width: borderWidth,
      height: 0.4,
      depth: boardHeight,
    }, scene),
